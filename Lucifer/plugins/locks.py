...
from Lucifer.plugins.sql_helper.locks_sql import get_locks, is_locked, update_lock
from Lucifer.utils import admin_cmd


@Lucifer.on(admin_cmd(pattern=r"lock( (?P<target>\S+)|$)"))
@Lucifer.on(sudo_cmd(pattern=r"lock( (?P<target>\S+)|$)", allow_sudo=True))
async def _(event):
...
@Lucifer.on(admin_cmd(pattern="unlock ?(.*)"))
@Lucifer.on(sudo_cmd(pattern="unlock ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@Lucifer.on(admin_cmd(pattern="currenabledlocks"))
@Lucifer.on(sudo_cmd(pattern="currenabledlocks", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    res = ""
...
@Lucifer.on(events.ChatAction())  # pylint:disable=E0602
async def _(event):
    # TODO: exempt admins from locks
    # check for "lock" "bots"
