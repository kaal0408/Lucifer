...
@Lucifer.on(admin_cmd(pattern=r"remove\.bg ?(.*)"))
@Lucifer.on(sudo_cmd(pattern=r"remove\.bg ?(.*)", allow_sudo=True))
async def _(event):
    ...
    if "image" in contentType:
        ...
        await eor(
            event,
            "Removed dat annoying Backgroup in {} seconds, powered by @Lucifer_support_group".format(
                ms
            ),
        )
    else:
