...
@Lucifer.on(admin_cmd(pattern="botgban ?(.*)"))
@Lucifer.on(sudo_cmd(pattern="botgban ?(.*)", allow_sudo=True))
async def _(event):
...
@Lucifer.on(admin_cmd(pattern="botungban ?(.*)"))
@Lucifer.on(sudo_cmd(pattern="botungban ?(.*)", allow_sudo=True))
async def _(event):
    if Config.G_BAN_LOGGER_GROUP is None:
        await eor(
            event,
