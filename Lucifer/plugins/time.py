...
@Lucifer.on(admin_cmd(pattern="time ?(.*)"))  # pylint:disable=E0602
@Lucifer.on(sudo_cmd(pattern="time ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    await borg.send_file(  # pylint:disable=E0602
        event.chat_id,
        required_file_name,
        caption="Lucifer: Powered by @Lucifer_support_group",
        # Courtesy: @ManueI15
...
@Lucifer.on(admin_cmd(pattern="gtime (.*)"))  # pylint:disable=E0602
@Lucifer.on(sudo_cmd(pattern="gtime (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    input_str = event.pattern_match.group(1)
