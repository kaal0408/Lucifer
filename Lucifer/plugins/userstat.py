...
@Lucifer.on(admin_cmd(pattern=f"ustat(?: |$)(.*)"))
@Lucifer.on(sudo_cmd(pattern=f"ustat(?: |$)(.*)", allow_sudo=True))
async def _(event):
    sender = await event.get_sender()
    me = await event.client.get_me()
    if not sender.id == me.id:
        Lucifer = await event.reply("`Processing...`")
    else:
        Lucifer = await event.edit("`Processing...`")
    if event.fwd_from:
        return
