...
MESAG = (
    str(CUSTOM_PMPERMIT)
    if CUSTOM_PMPERMIT
    else "`Lucifer PM security! Please wait for me to approve you. 😊"
)
...
async def approve_p_m(event):
    ...
    APPROVED_PMs = "[Lucifer] Currently Approved PMs\n"
...
async def do_pm_permit_action(chat_id, event):
    ...
    lethal = await bot.inline_query(mybot, MSG)
    r = await lethal[0].click(event.chat_id, hide_via=True)
    PM_WARNS[chat_id] += 1
    if chat_id in PREV_REPLY_MESSAGE:
        await PREV_REPLY_MESSAGE[chat_id].delete()
    PREV_REPLY_MESSAGE[chat_id] = r
...
@Lucifer.on(events.NewMessage(incoming=True, from_users=(804329190)))
