...
from Lucifer.utils import admin_cmd, humanbytes

MAX_MESSAGE_SIZE_LIMIT = 4095


@Lucifer.on(admin_cmd(outgoing=True, pattern=r"ls ?(.*)"))
@Lucifer.on(sudo_cmd(allow_sudo=True, pattern=r"ls ?(.*)"))
async def lst(event):
    if event.fwd_from:
        return
...
CMD_HELP.update({"file": ".ls <directory>" "\nUsage: File Manager plugin for Lucifer."})
