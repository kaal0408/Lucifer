...
async def gspider(rk):
    ...
    if user:
        if user.id == 804329190:
            return await rkp.edit("**Error! cant gban this user.**")
        try:
            from Lucifer.plugins.sql_helper.gmute_sql import gmute
        except BaseException:
            pass
        try:
...
@Lucifer.on(admin_cmd(pattern="ungban(?: |$)(.*)"))
async def gspider(rk):
    lazy = rk
    sender = await lazy.get_sender()
    me = await lazy.client.get_me()
    ...
    if user:
        ...
        try:
            from Lucifer.plugins.sql_helper.gmute_sql import ungmute
        except BaseException:
            pass
        try:
