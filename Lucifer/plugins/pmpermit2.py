    ...
    async def permitpm(event):
        ...
        if not apprv and event.text != UND:
            if Redis("MOVE_ARCHIVE") == "True":
                try:
                    await Lucifer.edit_folder(user.id, folder=1)
                except BaseException:
                    pass
            if event.media:
                await event.delete()
            ...
            if COUNT_PM[user.id] >= WARNS:
                ...
                await Lucifer(BlockRequest(user.id))
                await Lucifer(ReportSpamRequest(peer=user.id))
...
@Lucifer_cmd(pattern="block ?(.*)", fullsudo=True)
...
@Lucifer_cmd(pattern="listapproved")
async def list_approved(event):
    ...
    for i in all:
        try:
            name = (await Lucifer.get_entity(i)).first_name
        except BaseException:
            name = ""
...
async def apr_in(event):
    ...
    if not is_approved(uid):
        approve_user(uid)
        try:
            await Lucifer_bot.edit_folder(uid, folder=0)
        except BaseException:
            pass
        try:
...
async def in_pm_ans(event):
    ...
    if not res:
        ...
        res = [
            event.builder.article(
                ...
                type=_type,
                text=msg_,
                description="@Lucifer_support_group",
    ...
    await event.answer(res, switch_pm="• Lucifer •", switch_pm_param="start")
