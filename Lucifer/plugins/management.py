...
@Lucifer.on(admin_cmd(pattern="unbanall ?(.*)"))
@Lucifer.on(sudo_cmd(pattern="unbanall ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@Lucifer.on(admin_cmd(pattern="skick ?(.*)"))
@Lucifer.on(sudo_cmd(pattern="skick ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    if event.is_private:
