...
from Lucifer import CMD_HELP, CMD_HNDLR

Heroku = heroku3.from_key(Var.HEROKU_API_KEY)
heroku_api = "https://api.heroku.com"


@Lucifer.on(admin_cmd(pattern=r"(set|get|del) var (.*)", outgoing=True))
...
@Lucifer.on(admin_cmd(pattern="usage"))
@Lucifer.on(sudo_cmd(pattern="usage", allow_sudo=True))
...
@Lucifer.on(admin_cmd(pattern="info heroku"))
@Lucifer.on(sudo_cmd(pattern="info heroku", allow_sudo=True))
...
@Lucifer.on(admin_cmd(outgoing=True, pattern=r"logs"))
@Lucifer.on(sudo_cmd(allow_sudo=True, pattern=r"logs"))
async def _(givelogs):
    ...
    with open("logs-Lucifer.txt", "w") as log:
