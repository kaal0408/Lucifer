...
@Lucifer.on(admin_cmd(pattern="info ?(.*)"))
@Lucifer.on(sudo_cmd(pattern="info ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    async with event.client.conversation(chat) as conv:
        ...
        if response1.text.startswith("No records found"):
            ...
            await event.delete()
            await Lucifer.send_message(event.chat_id, response2.message)

            await Lucifer.send_message(event.chat_id, response3.message)


CMD_HELP.update(
    {
        "sangmata": ".info <reply to user>\nUse - Get full name and username history of the person."
    }
)
