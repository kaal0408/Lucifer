...
@Lucifer.on(admin_cmd(outgoing=True, pattern="abuse"))
async def abuse(event):
  strings = random.choice(abuse)
  await Lucifer.send_message(event.chat_id, strings)
  
#   Rapid 50 abuse 
@Lucifer.on(admin_cmd(outgoing=True, pattern="raabuse"))
async def raid(event):
  for i in range(0, 50):
    random_string = random.choice(abuse)
    await Lucifer.send_message(event.chat_id, random_string)
