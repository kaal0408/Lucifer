...
@Lucifer.on(admin_cmd(pattern="btweet(?: |$)(.*)"))
@Lucifer.on(sudo_cmd(pattern="btweet(?: |$)(.*)", allow_sudo=True))
async def teletweet(telebot):
    # """Creates random anime sticker!"""
    what = telebot.pattern_match.group(1)
    if not what:
        if Lucifer.is_reply:
            what = (await Lucifer.get_reply_message()).message
        else:
            await eor(Lucifer, "`Tweets must contain some text, pero!`")
            return
    sticcers = await bot.inline_query("TwitterStatusBot", f"{(deEmojify(what))}")
    await sticcers[0].click(
        telebot.chat_id,
        reply_to=Lucifer.reply_to_msg_id,
        silent=True if Lucifer.is_reply else False,
        hide_via=True,
    )
...
async def tweet(uname, mssg):
    ...
    the_pic.save("Lucifer.jpg", "jpeg")
    return "Lucifer.jpg"


@Lucifer.on(admin_cmd(pattern="tweet ?(.*)"))
@Lucifer.on(sudo_cmd(pattern="tweet ?(.*)"))
async def handler(event):
    ...
    await Lucifer.send_file(event.chat_id, pic_tweet, reply_to=reply_to)
    await event.delete()
